    ...
    'Backup' {
        ...
        try {
            $bak = Backup-DbaDatabase -SqlInstance $inst -Database $db `
                    -Path $dest -Type Full -CopyOnly -Checksum -Verify `
                    -CompressBackup -FilePath "$db`_Decom_Full_$(Get-Date -Format yyyyMMdd).bak"
            Write-DecomLog $log "Backup file : $($bak.BackupPath)"
            Write-DecomLog $log "Size        : $($bak.TotalSize)"
            Write-DecomLog $log "Verified    : $($bak.Verified)"
        }
        finally {
